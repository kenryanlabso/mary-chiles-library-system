<?php
	...
	$file_name = date("m-d-y_").ucwords($report_type);

	$query_admin = "SELECT * FROM account_tbl INNER JOIN employee_tbl ON account_tbl.account_number = employee_tbl.account_number WHERE account_tbl.account_type = 'Admin' AND account_tbl.account_number = '$admin_id'";
	$result_admin = $con->query($query_admin);
	$row_admin = $result_admin->fetch_assoc();
	$admin_name = strtoupper($row_admin['employee_fname']." ".$row_admin['employee_lname']);
...
<body>	
	...
	<div class="print-contents">
		...
		<div class="report-header">
			<img src="../images/logo.png" height="70" style="position:relative; margin-bottom:10px;"/><br/>
			<b>MARY CHILES COLLEGE LIBRARY</b><br>
			#667 Dalupan Street,<br>
	...
	<?php
		if($report_type == "acquisition")
		{
			...
			<div class="report-footer">
				...
				<?php echo "<u>&nbsp;&nbsp;&nbsp;".$admin_name."&nbsp;&nbsp;&nbsp;</u><br/>"; ?>
				<b>College Librarian</b><br/>
				<?php echo date("m/d/Y"); ?>
			</div>
			<?php
				$report_content = ob_get_clean();
				$pdf->loadHtml($report_content);
		...
		else if($report_type == "reservations")
		{
			?>
			<div id="report-label">
				REPORT ON RESERVATIONS OF BOOKS
			</div>
			<div id="report-date">
				<?php echo date("F d, Y", strtotime($start_date))." - ".date("F d, Y", strtotime($end_date)); ?>
			</div>
			<table class="table">
				...
				<?php	
				$query_student = "SELECT * FROM book_tbl INNER JOIN reservation_tbl ON book_tbl.book_id = reservation_tbl.book_id INNER JOIN student_tbl ON reservation_tbl.account_number = student_tbl.account_number WHERE reservation_tbl.reserve_status = 'Pending' AND reservation_tbl.reserve_date BETWEEN '$start_date' AND '$end_date'";
				$result_student = $con->query($query_student);
				while($row_student = $result_student->fetch_assoc())
				{
				...
				$query_employee = "SELECT * FROM book_tbl INNER JOIN reservation_tbl ON book_tbl.book_id = reservation_tbl.book_id INNER JOIN employee_tbl ON reservation_tbl.account_number = employee_tbl.account_number WHERE reservation_tbl.reserve_status = 'Pending' AND reservation_tbl.reserve_date BETWEEN '$start_date' AND '$end_date'";
				$result_employee = $con->query($query_employee);
				while($row_employee = $result_employee->fetch_assoc())
				{
		...
		else if($report_type == "user-registration")
		{
			...
			<table class="table">
				...
				<?php
					$query_borrower = "SELECT * FROM student_tbl LEFT JOIN account_tbl ON student_tbl.account_number = account_tbl.account_number LEFT JOIN course_tbl ON student_tbl.course_id = course_tbl.course_id WHERE account_tbl.account_registered BETWEEN '$start_date' AND '$end_date' AND account_tbl.account_type = 'Regular'";
